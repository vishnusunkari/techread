...
Here is how the data persisted on the SQLite Database with and without acknowledgements where
Acknowledged is status=5    (when preId = q.ack(task) is enabled in the above code)     
Unacknowledged is status=2  (when preId = q.ack(task) is commented in the above code)       
